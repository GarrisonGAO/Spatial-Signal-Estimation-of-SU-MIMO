...
def ml_estimation(H, Y):
    ...
    for X in all_combinations:
        X = X.reshape(-1, 1)  # 调整为列向量
        Y_hat = np.dot(H, X)  # 计算接收信号
        distance = np.linalg.norm(Y - Y_hat)  # 计算与实际接收信号的距离
...
def channel_matrix_has_LOS(Dr, Dt, L_total, Nr, Nt, Nr_virtual):
    ...
    # 这里好在 diagonal matrix 本身和 Nr, Nt 其实是没有关系的，只和 L 路径数量有关，带上 Nt 是为了方便进行归一化
...
Nt = 30  # antenna number of transmitter array
L_total = 50  # number of scatters
...
for _ in range(num_samples):
    # 现在需要的是输入， Rx_symbol_fix 和全套的信道矩阵 H_total_fix_vir,输出是 Rx_symbol_vir
    H_total_fix_vir, H_total_fix = channel_matrix_has_LOS(Dr=Dr, Dt=Dt, L_total=L_total, Nr=Nr, Nt=Nt, Nr_virtual=Nr_virtual)
    Tx_symbol = QPSK(Nt, L_total)
    noise = generate_complex_gaussian_noise(N=Nr+Nr_virtual, mean=0, variance=0.01)
    Rx_symbol_fix_vir = H_total_fix_vir @ Tx_symbol

    average_energy = signal_energy(Rx_symbol_fix_vir)
    energys.append(average_energy)

print(np.mean(energys))
